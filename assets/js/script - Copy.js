...
function callAPI(cityValue, countryValue){
    //let city_name = $('#city-name').val();
    //let country_code = $('#country-name').val();
    ...
    let requestUrl_LatLon = `http://api.openweathermap.org/geo/1.0/direct?q=${combined}&limit=${limit}&appid=${API_KEY}`;
...
function displayWeather(cityValue, weatherList) {
    ...
    for (var i = 0; i < weatherList.length; i++) {
        ...
        if (weatherList[i].dt < tomorrowDay && !displayedTodays) {
            ...
            if (weatherList[i].dt_txt.slice(11, 13) == "12") {
...
function saveNewCity(cityValue, countryValue){
    
    //get array of stored cities
    var storedCities = JSON.parse(localStorage.getItem("cities"));
    console.log("proper case = " + cityValue.toProperCase());
    //create object from current city and country
    var currentCity = {
...
function createNewButton(cityValue, countryValue) {
    var newButton = document.createElement("button");
    newButton.appendChild(document.createTextNode(cityValue.toProperCase() + " (" + countryValue + ")"));
...
function createAllButtons() {
    ...
    if (storedCities != null) {
        ...
        for (let i=0; i<storedCities.length; i++){
            //console.log(storedCities[i].city + "/" + storedCities[i].country );
            createNewButton(storedCities[i].city, storedCities[i].country);
        }
    }
}

function inStorage(cityValue, countryValue) {
   // alert('Check storage');
    var storedCities = JSON.parse(localStorage.getItem("cities"));
    if (storedCities == null) {
    ...
    for (let i=0; i<storedCities.length; i++){
        if (storedCities[i].city === cityValue.toProperCase() && storedCities[i].country === countryValue.toUpperCase()) {
...
function submitButtonHandler (event){
   // alert('submit button handler');
    ...
    if (validateFields("city-name")) {
       // alert('check storage');
        if (!inStorage(cityValue, countryValue)) {
            createNewButton(cityValue, countryValue);
            saveNewCity(cityValue, countryValue);
        }
        callAPI();
        printStorage();
        $('#city-name').val("");
...
function allButtonsHandler(event) {
    ...
    console.log("allButtonsHandler" + " / " + cityValue + " / " + countryValue);
    callAPI(cityValue, countryValue);
}
